...
async def main():
    ...
    def reset_app():
        for key in ['history', 'past', 'generated', 'sources', 'ready', 'file_hash', 'doc_name', 'response_times', 'metrics', 'retriever']:
    ...
    if uploaded_file is not None:
        ...
        if st.session_state['file_hash'] != current_hash or not st.session_state['ready']:
            ...
            try:
                ...
                # Make sure the welcome message is only displayed after retriever is definitely set
                if 'retriever' in st.session_state and st.session_state['retriever'] is not None:
                    ...
                    # If retriever isn't set, don't show ready message
                    st.session_state['ready'] = False
                    raise Exception("Failed to initialize document retriever properly")
    ...
    if st.session_state['ready'] and 'retriever' in st.session_state and st.session_state['retriever'] is not None:
        ...
        # Display an error message and troubleshooting info if there's no document loaded
        if uploaded_file is None:
            ...
            # We have a document but the retriever isn't ready
            st.error("⚠️ Document processing incomplete. Please try reloading the document or check for errors.")
            st.markdown("""
            ### Troubleshooting:
            1. **Try reloading the document** by clicking "Reset App" in the sidebar and uploading again
            2. **Check document format** - make sure your PDF contains searchable text
            3. **Document size** - very large documents may take longer to process
            4. **Document complexity** - documents with complex formatting may cause issues
            """)
