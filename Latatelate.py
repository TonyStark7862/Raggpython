            ...
            with st.spinner("Thinking..."):
                try:
                    ...
                    if use_vector_only:
                        ...
                        if FASTEMBED_AVAILABLE:
                            try:
                                ...
                                st.warning(f"Hybrid search failed, falling back to text search: {e}")
                            ...
                            try:
                                ...
                                # Fallback to standard vector search if text search fails
                                st.warning(f"Text search failed, falling back to vector search: {e}")
                                search_results = qdrant_client.search(
                                    collection_name=st.session_state.collection_name,
                                    query_vector=query_embedding.tolist(),
                                    limit=limit,
                    ...
                    if not sources:
                        ...
                        # Get response from your custom LLM function
                        try:
                            # Check if abc_response is defined (this was in the original code)
                            if 'abc_response' in globals():
                                response = abc_response(full_prompt)
                            else:
                                # Fallback message if no LLM function is available
                            ...
                            response = f"LLM processing error: {str(e)}\n\nHowever, I found these relevant passages in the document that might help answer your question."
