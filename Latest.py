...
def ensemble_search(collection_name, query_text, limit=4, vector_weight=0.7, use_reranking=True):
    ...
    try:
        ...
        hybrid_results = qdrant_client.search(
            collection_name=collection_name,
            query_vector=query_embedding.tolist(),  # Required positional argument
            limit=limit,
            with_payload=True,
            # Use hybrid search capabilities from Qdrant
            append_payload=True,  # Include payload in results
            with_vectors=False,   # No need to return vectors
            # Text search configuration
            score_threshold=0.0,
            search_params=models.SearchParams(
                hnsw_ef=128,  # Controls recall at the expense of latency
                exact=False   # Set to True for more accurate but slower search
            ),
            # Keyword text matching on content field
            query_filter=None,
            # Use text search
            metadata={"text": query_text, "field": "content"}
        )
                ...
                try:
                    ...
                    if use_vector_only:
                        ...
                        search_results = qdrant_client.search(
                            ...
                            query_filter=None,
                            # Use text search
                            metadata={"text": prompt, "field": "content"}
                        )
                        sources = [result.payload["text"] for result in search_results]
                        metadata = [{
                            "score": result.score,
