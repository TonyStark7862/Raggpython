            ...
            with st.spinner("Thinking..."):
                try:
                    ...
                    if not sources:
                        ...
                        # Get response from your custom LLM function
                        try:
                            # Check if abc_response is defined
                            if 'abc_response' in globals():
                                response = abc_response(full_prompt)
                            else:
                                # Fallback if no LLM function is available
                                response = f"Using local LLM to answer: {prompt}\n\nBased on the document, I found relevant information that would help answer your question."
                        except Exception as e:
